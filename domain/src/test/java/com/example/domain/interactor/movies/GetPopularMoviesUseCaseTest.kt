...
class GetPopularMoviesUseCaseTest {
    lateinit var popularMoviesUseCase: GetPopularMoviesUseCase
    lateinit var popularMoviesRepository: PopularMoviesRepository

    @Before
    fun init() {
        popularMoviesRepository = mockk()
        popularMoviesUseCase = GetPopularMoviesUseCase(popularMoviesRepository)
    }
    ...
    fun `given the repository returns a successful Result, run() should return a successful result`() = runBlockingTest {
        ...
        //when
        val result: Result<List<Movie>> = popularMoviesUseCase.run()
